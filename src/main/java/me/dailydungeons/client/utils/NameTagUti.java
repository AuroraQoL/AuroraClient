...
/*        if (mc.gameSettings.thirdPersonView == 2) {
        ...
        final int var16 = mc.fontRendererObj.getStringWidth(entityName) / 2;
    //    RenderUtil.rectangle(-var16 - 3, var15 - 3, var16 + 3, var15 + 10, new Color(32, 32, 33, 120).getRGB());
        ...
        if (player instanceof EntityPlayer) {
            ...
            for (final ItemStack stack2 : items) {
                ...
                mc.getRenderItem().renderItemIntoGUI(stack2, -var16 + offset + xPos, var15 - 20);
                mc.getRenderItem().renderItemOverlays(mc.fontRendererObj, stack2, -var16 + offset + xPos,
                        var15 - 20);
                ...
                if (stack2.getItem() == Items.golden_apple && stack2.hasEffect()) {
                    ...
                    if (enchants != null) {
                        int encY = 0;
                        final Enchantment[] important = new Enchantment[]{Enchantment.sharpness,
                                Enchantment.unbreaking, Enchantment.fireAspect, Enchantment.fireAspect,
                                Enchantment.efficiency, Enchantment.fireProtection, Enchantment.power,
    ...
    public static void renderArmor(final EntityPlayer player) {
        ...
        for (int index = 3; index >= 0; --index) {
            ...
            renderItemStack(armourStack2, xOffset);
            xOffset += 16;
        }
    }
    ...
    private static float getSize(final EntityPlayer player) {
        final EntityPlayerSP ent = mc.thePlayer;
        final double dist = ent.getDistanceToEntity(player) / 5.0f;
        return dist <= 2.0 ? 1.3f : (float) dist;
    }

    private static void renderItemStack(final ItemStack stack, final int x) {
        GlStateManager.pushMatrix();
